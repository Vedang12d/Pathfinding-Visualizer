...
export function bestFirstSearch(grid, startNode, finishNode) {
  ...
  while (!unvisitedNodes.isEmpty()) {
    ...
    if (!nextNode.wallNode) {
      ...
      //Reached target node
      if (
        nextNode.row === finishNode.row &&
        nextNode.column === finishNode.column
      ) {
        return visitedNodes;
      }
...
function updateNeighbourNodes(
  ...
  const filteredNeighbourNodes = neighbourNodes.filter(
    (node) => !node.visitedNode
  );
  filteredNeighbourNodes.forEach((node) => {
    if (
      Math.abs(node.row - currNode.row) === 1 &&
      Math.abs(node.column - currNode.column) === 1
    ) {
    ...
    let hValue = node.distance + euclideanDistance(node, finishNode);
  ...
  filteredNeighbourNodes.forEach((node) => {
    ...
    if (duplicatesSet[hValue] > 1) {
      unvisitedNodes.enqueue(
        node,
        euclideanDistance(node, finishNode) +
          calcTiebreaker(currNode, startNode, finishNode)
      );
    } else {
      unvisitedNodes.enqueue(node, euclideanDistance(node, finishNode));
    }
  });
}
...
function manhattanDistance(currNode, finishNode) {
...
function euclideanDistance(currNode, finishNode) {
  const deltaX = Math.abs(currNode.row - finishNode.row);
  const deltaY = Math.abs(currNode.column - finishNode.column);
