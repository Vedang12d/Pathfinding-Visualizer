...
function addWall(dir, num, vertical, horizontal, startNode, finishNode) {
  ...
  if (dir === 0) {
    ...
    for (let temp of horizontal) {
      if (
        (temp === startNode.row && num === startNode.column) ||
        (temp === finishNode.row && num === finishNode.column)
      ) {
        isStartFinish = true;
        continue;
      }
    ...
    for (let temp of vertical) {
      if (
        (num === startNode.row && temp === startNode.column) ||
        (num === finishNode.row && temp === finishNode.column)
      ) {
        isStartFinish = true;
        continue;
      }
